...
class GetMotifs extends CI_Controller {
  ...
  { 
    ...
    $return_messages = [];
    $dbhandle = mysqli_connect("mysql","rlswor5_richard",'Syp3rtjr2#@!');  //connect to mysqli
    array_push($return_messages,"Connection to MySQL Made");
    if($dropDb == 'yes')
      {
        ...
        if (!mysqli_error($dbhandle)) 
          {
            array_push($return_messages,"Database rlswor5_proteins was successfully dropped");
          } 
        else if(mysqli_error($dbhandle)) 
          {
            array_push($return_messages, 'Error dropping database: ' . mysqli_error($dbhandle)); 
          }

        echo json_encode(array("type" => "drop","messages" => $return_messages));
      }
        ...
        if($file != 'none') 
          {
            ...
            //echo $fileString;
            $this->buildProteinDb($fileString,$dbhandle,$return_messages);   // call function to Build Protein Database
          }
      }
...
public function buildProteinDb($filestr,$dbhandle,$return_messages)  // function for building Protein Database
{
  $fileString=$filestr;
  $conn = 0;
  ...
  //$dbhandle = mysqli_connect("mysql","rlswor5_richard",'Syp3rtjr2#@!');
 ...
 // else
  //  {
      ...
      array_push($return_messages,"Now Checking Database!");
      ...
      if (mysqli_query($dbhandle,$sql)) {
        if (mysqli_warning_count($dbhandle) == 0) { 
            array_push($return_messages,"Database created successfully!");
            echo "Database created sucessfully!";
                }
        } else {
            array_push($return_messages,"Error creating database: " . mysqli_error($dbhandle));
      ...
      if(mysqli_query($dbhandle,$sql)) {
        if(mysqli_warning_count($dbhandle) == 0){
          ...
          array_push($return_messages,"Protein Table already exist!");
          echo "Protein Table already exist!";
        }
      }
      
      $this->buildMinimotifDatabase($dbhandle,$return_messages);
      //mysqli_close($dbhandle); 
    //}
   
  }     // End of buildProteinDb function

public function buildMinimotifDatabase($dbhandle,$return_messages)
{
...
// Create mini-Motif database
   ...
   $sql = "CREATE TABLE miniMotif (id int not null primary key auto_increment,motifPattern varchar(75),actualMotif varchar(1500),accessionNumber varchar(75),speciesName varchar(75), proteinLength int, motifLength int, startPosition int, endPosition int )";
    ...
    mysqli_query($dbhandle, $sql);
      ...
      echo json_encode(array("type" => "build","messages" => $return_messages)); 
