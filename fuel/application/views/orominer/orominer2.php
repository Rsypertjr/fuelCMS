...
<head>
						...
						for(var sn=0;sn<noSysNodes;sn++)
							{
								...
								text.setAttributeNS(null,"y",(y1+150));
								text.setAttributeNS(null,"fill","black");
								text.setAttributeNS(null,"text-anchor","middle");
								text.setAttributeNS(null,"opacity",1);  
								text.setAttributeNS(null,"font-size",80);
									...
									for(var o2=0;o2<noOrgNodes;o2++)
										{
											...
											text.setAttributeNS(null,"y",(y2-80));
											text.setAttributeNS(null,"fill","black");
											text.setAttributeNS(null,"text-anchor","middle");
											text.setAttributeNS(null,"opacity",1);  
											text.setAttributeNS(null,"font-size",80);
													...
													for(pi=0;pi<noPartNodes;pi++)
														{
															...
															text.setAttributeNS(null,"y",(y2b-60));
															text.setAttributeNS(null,"fill","black");
															text.setAttributeNS(null,"text-anchor","middle");
															text.setAttributeNS(null,"opacity",1);  
		...
		{ 
			...
			function messageMenu(){
				$grhdr = $('#grahdr').css('display','block'); // Graph Header object]
					$grhdr.html("<div id='intromess'><span>Note: Hover Mouse Below to Enlarge Graph Area</span></div>");	

					// Revised Zoom Menu
					var b1 = $('<button id="zoomup" style="position:relative;float:right;margin:-1em 0 0 -2em">Zoom Up</button>');
					var b2 = $('<br><button id="zoomdwn" style="position:relative;float:right;margin:-3em 0 0 -1em">Zoom Down</button>');
					var txt = $('<br><span>Note: Hold Mouse Key Down on Image to Move it</span>');
					$('#intromess').css("margin","0");
					$('#intromess').append(txt);					
					$grhdr.attr("style","margin:0").append(b1).append(b2);
					...
					$('#zoomup').click(function()
					{
						...
						let winSVG = $("#mySVG");			
						let gphdisp2 = winSVG.find("#outerg");
							...
							$odd.on('mouseup',{numOrg:numOrgans},function(e) 
							{
								...
								if(prompt == "Click on Box to Open")
								{
									...
									else if(e.data.numOrg == 0)
									{
											...
											$('#intromess').html("<div>Sorry, System: <span style='color:"+systemcolor[indx2[1]]+"'>"+nme+"</span> Has NO ORGANS!!</div>");
									}
													...
													if(numCells>0 && openthispartscells)   
														...
														for (m=0;m<numCells;m++)
															{  
																...
																$id3.css('float','right').css('margin','1em 1em 0 0');	
																...
																$odd.bind('mouseenter',function()
																{
																		$(this).find("div:contains('Click on Box to Open')").css('zoom','105%').css('color','red');
																		$(this).parent().siblings().find('.cell_id2').find("div:contains('Click on Box to Open')").css('color','black'); 
																})
																.bind('mouseleave',function()
																	{
																			$(this).find("div:contains('Click on Box to Open')").css('zoom','100%').css('color','black');
																			$(this).parent().siblings().find('.cell_id2').find("div:contains('Click on Box to Open')").css('color','black'); 
																	})	   
																.bind('mousedown',function()
																	{
																...
																$odd.on('mousedown',function(e) 
																	{
																		...
																		$grhdr.css('z-index','2');
																		var prompt = $(this).find("div:contains('Click')").text();
																		if(prompt == "Click on Box to Open")	
																			{
	...
	$(document).ready(function()   // loads html of gui on body load
	{
            ...
            $('#outrdiv').css("height",'20em');			
			$('#hierdiv').css('zoom','65%');
			$('#imgview').css('zoom','60%');

			// Expansion of Graph View
			$('#grview').on('mouseenter mouseover',function()
				{
					$('#hierdiv').css('display','none');
					$(this).css('width','80%').css('margin-right','2%');
				...
				.on('mouseleave',function()
				{
					...
					$('#hierdiv').css('display','block');
					$(this).css('width','45%').css('margin-right','0%');	
