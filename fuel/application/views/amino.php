...
function valInput(dataType = null)
{
...
if(!fnm.match(/(\w|\d|(\Q-_\E))*\.txt/))
  {
...
else
   {
		...
		makeRequest(fnm,rcvid,"no",mess,data_type=dataType);
...
function makeRequest(snd,recid,drp,mess,data_type="json")
{
	if(data_type == "json")
		mobileLoader(mess);
	$.ajax({
		   ...
		   success: function(result) {
			...
			if(result.type == 'drop' || result.type == 'build'){
				$('#'+recid).html('');
				for (message of result.messages){
					$('#'+recid).append(`<p>${message}</p>`);
				}
			}
			else
				$('#'+recid).html(result);
		...
		//	$('#'+recid).html('').append(`${data}`).append(`An error occured: ${xhr.status} ${xhr.statusText}`);
       //    }
				...
				<div id="fileinput" >
					<form id="filein">
						<div id="filecapt">
							...
							<div class="aButton2" type="button"  onclick="valInput('text')">Verify File and Start Database Build</div>
