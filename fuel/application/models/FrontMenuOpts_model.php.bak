...
class FrontMenuOpts_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'FrontMenuOpt'; // the name of the record class (if it can't be determined)
	...
	{
		parent::__construct('FrontMenuOpts'); // table name
	}

	public function list_items($limit = NULL, $offset = NULL, $col = 'precedence', $order = 'asc', $just_count = FALSE)
	{
		// $this->db->select('FrontMenuOpts.id, FrontMenuOpts.title, FrontMenuOpts.published....'); uncomment and  change to the fields to be displayed
...
class FrontMenuOpt_model extends Base_module_record {
